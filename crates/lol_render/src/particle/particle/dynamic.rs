...
use crate::shader::{SharedRenderData, SharedSamplerDef};

use crate::particle::shader_layout::{BindingTypeDesc, ShaderLayoutDescriptor};
use crate::particle::{
    ATTRIBUTE_LIFETIME, ATTRIBUTE_UV_FRAME, ATTRIBUTE_WORLD_POSITION, ATTRIBUTE_WORLD_POSITION_VEC4,
};
use crate::shader::ShaderMap;
...
impl ParticleMaterialDynamic {
    ...
    pub fn write_after_member(
        ...
        let mut hit = false;
        for desc in [self.vert_layout.clone(), self.frag_layout.clone()] {
            for binding in desc.bindings.values() {
