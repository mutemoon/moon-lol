...
fn main() {
    ...
    println!("环境重置完成. 智能体数: {}, 初始观测维数: {}", obs.len(), SoloV0Env::state_dim());
    ...
    for t in 0..160 {
        ...
        let fiora_pos = env.world().get::<Transform>(env.fiora()).map(|t| t.translation).unwrap_or_default();
        let riven_pos = env.world().get::<Transform>(env.riven()).map(|t| t.translation).unwrap_or_default();

        // 统计距离 Fiora 最近的小兵
        let mut minion_dists: Vec<(f32, f32, f32, lol_core::team::Team)> = minion_info.iter().map(|(_, pos, hp, max, team)| {
            (fiora_pos.distance(*pos), *hp, *max, *team)
        }).collect();
        ...
        if t % 20 == 0 || t < 10 {
            println!(
                ...
                t, fiora_pos.x, fiora_pos.z, riven_pos.x, riven_pos.z, minion_info.len(),
                ...
                minion_dists.first().map(|m| m.3).unwrap_or(lol_core::team::Team::Order),
            );
        }
        ...
        if let Some(&cs) = res[0].reward_variables.get("self_cs") {
            if cs > 0.0 {
                fiora_cs += cs;
                println!("🎯 [Step {:3}] Fiora 成功击杀小兵！当前 CS: {:.0}", t, fiora_cs);
            }
        }
        if let Some(&cs) = res[1].reward_variables.get("self_cs") {
            if cs > 0.0 {
                riven_cs += cs;
                println!("🎯 [Step {:3}] Riven 成功击杀小兵！当前 CS: {:.0}", t, riven_cs);
            }
        }
    ...
    println!("\n对局总结: Fiora CS = {}, Riven CS = {}", fiora_cs, riven_cs);
}
