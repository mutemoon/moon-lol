...
fn test_multi_agent_selfplay_signatures_and_throughput() {
    ...
    let expected_dim = SoloV0Env::state_dim();
    ...
    assert_eq!(initial_obs[0].to_vector().len(), expected_dim);
    assert_eq!(initial_obs[1].to_vector().len(), expected_dim);
    ...
    assert_eq!(step_res[0].obs.to_vector().len(), expected_dim);
    assert_eq!(step_res[1].obs.to_vector().len(), expected_dim);
}
