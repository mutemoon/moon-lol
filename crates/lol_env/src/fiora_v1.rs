...
pub fn dispatch_action_world(
    ...
    if !action.attack {
        world.trigger(lol_core::attack_auto::CommandAttackAutoStop { entity: fiora });
...
pub fn advance_action_simulation(
    ...
    if action.attack {
        ...
        let rpos = app
            ...
            .unwrap_or_default();
        let target = Vec3::new(
            rpos.x + action.move_x.clamp(-1.0, 1.0) * MOVE_SCALE,
            rpos.y,
            rpos.z + action.move_z.clamp(-1.0, 1.0) * MOVE_SCALE,
        );
