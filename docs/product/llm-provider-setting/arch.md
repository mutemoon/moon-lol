...
### 桌面端编排器 [agent.rs](/apps/client/src-tauri/src/agent.rs)

- `AgentConfig` 含 `model` / `provider_id`；`Orchestrator::new` 调 `load_providers` 读 `~/.moon-lol/providers.json`，`resolve_credentials` 按 `provider_id` 解析每选手凭证，`provider_id` 为空时走平台 env。
- 与服务端编排器保持一致语义。
...
| 兼容代理 | [backend.ts](/apps/client/src/services/backend.ts) | 桌面端走 `get_model_providers`/`set_model_providers` 整存 |
| Tauri 命令 | [lib.rs](/apps/client/src-tauri/src/lib.rs) | `ModelProvider` 结构 + `get_model_providers`/`set_model_providers`，读写 `~/.moon-lol/providers.json` |
| 预设目录 | [providerPresets.ts](/apps/client/src/config/providerPresets.ts) | 整理自 cc-switch 的厂商预设；`PLATFORM_PROVIDER_ID` 哨兵 |
| 状态 | [providersStore.ts](/apps/client/src/stores/providersStore.ts) | `useProviders`：Web 走云端 CRUD、桌面端走 providers.json 整存 |
...
- **Web**：走云端 CRUD（`/api/model-providers`）。
- **桌面端**：走本地 `~/.moon-lol/providers.json` 整存整取（对局运行时读取来源）。云端同步留作后续 todo。

## 六、数据流总览

```
设置页 ──CRUD──> providersStore ──> (Web: /api/model-providers ─> model_providers 表)
                                  (Desktop: providers.json)
...
启动对局 ──> 编排器.resolve_credentials(provider_id)
            ├── BYO: 按 provider_id 查 model_providers / providers.json 取凭证
