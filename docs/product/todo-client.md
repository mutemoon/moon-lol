...
- [x] **预设体系升级为 Agent / 大脑 / 出生点预设** — 已完成。前端页面（`heroes.vue` 作为 Agent 管理，`agents.vue` 作为大脑配置，`spawn-presets.vue` 作为出生点）已与三层模型（Agent = 英雄 + 大脑）完全对齐。
- [x] **胜利条件配置系统 UI** — 已完成。在主页中集成了 `WinConditionBuilder.vue` 和 `WinNode.vue`，支持配置条件原子与组合逻辑，并随场景保存。
- [x] **Script Agent 编辑器** — 已完成。`agents.vue` 在 `agent_type === 'script'` 时切换为 Monaco Editor（`@guolao/vue-monaco-editor` + `monaco-editor`），JS 语法高亮 + 热重载开关 + API（`observe()` / `action()` / `log()`）说明。断点、单步执行通过桌面端运行时承接。
...
## Phase 4 — Rank 竞技
- [x] **参赛快照发布机制 UI** — 已完成。`agents.vue` 顶部新增「发布参赛快照」按钮与可见性切换（私有/好友/公开），底部以列表展示历史快照（版本号 + 发布时间 + "最新"标记）。
...
## Phase 5 — 社区与职业
- [x] **Agent 社区市场 UI** — 已完成。`/community` Tabs 切换最新 / 热门 / ELO 排序，搜索框过滤 Agent / 英雄；卡片网格展示公开 Agent，点击触发 Fork 对话框（自定义新名称，Fork 成功后跳回 `/agents?focus=…`），Fork 出的副本卡片附 GitFork 徽章。
