  ...
  <div class="flex h-full w-full flex-col overflow-hidden bg-background p-4">
    ...
    >
      <div class="flex items-center gap-2.5">
        ...
        <div class="flex items-baseline gap-2">
          ...
          <Badge variant="secondary" class="text-[10px]">{{ spawnPresets.length }}{{ t("spawnPresets.countUnit") }}</Badge>
        </div>
      </div>
    ...
    <div class="mt-3 flex min-h-0 flex-1 gap-3">
      ...
      <section class="border-border bg-card min-w-0 flex-1 overflow-y-auto rounded-lg border p-5 shadow-sm">
        <div v-if="errorMsg" class="border-border mb-3 rounded border-l-2 px-3 py-1.5 text-xs"
          :class="errorMsg === t('spawnPresets.successSave') ? 'border-green-500 text-green-500 bg-green-500/5' : 'border-destructive text-destructive bg-destructive/5'">
          {{ errorMsg }}
        </div>
        ...
        <div class="mx-auto flex max-w-xl flex-col gap-4">
          ...
          <div class="bg-muted/40 border-border flex items-center justify-between rounded-md border px-3 py-2">
            <span class="text-muted-foreground text-[10px] font-semibold uppercase tracking-wider">{{ t("spawnPresets.coordsLabel") }}</span>
    ...
    <Dialog :open="showDeleteConfirm" @update:open="(v) => (showDeleteConfirm = v)">
      <DialogContent class="border-border bg-card text-foreground max-w-sm p-6">
        <DialogHeader>
          <DialogTitle class="text-foreground text-sm">{{ t("spawnPresets.deleteConfirmTitle", { name: selectedName }) }}</DialogTitle>
          <DialogDescription class="text-muted-foreground text-[11px]">
            {{ t("spawnPresets.deleteConfirmDesc") }}
          </DialogDescription>
        </DialogHeader>
        <DialogFooter class="gap-2">
          <Button variant="outline" size="sm" @click="showDeleteConfirm = false">{{ t("spawnPresets.cancelBtn") }}</Button>
          <Button variant="destructive" size="sm" @click="confirmDelete">{{ t("spawnPresets.confirmDeleteBtn") }}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  </div>
