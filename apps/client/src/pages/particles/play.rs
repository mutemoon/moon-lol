...
use super::state::{update_state, update_state_returns, with_state};
use crate::components::sidebar::AppSidebar;
use crate::services::particle_service;

pub(super) fn spawn_play_ron(cx: &mut Context<AppSidebar>, ron: String) {
...
pub(super) fn play_working(cx: &mut Context<AppSidebar>) {
...
pub(super) fn replay_after_edit(cx: &mut Context<AppSidebar>) {
...
pub(super) fn stop_playing(cx: &mut Context<AppSidebar>) {
    let handle = with_state(|s| s.ws_handle.clone());
    if let Some(h) = handle {
        let _weak = cx.entity().downgrade();
        cx.spawn(
...
pub(super) fn play_single_emitter(cx: &mut Context<AppSidebar>, idx: usize) {
    let ron = with_state(|s| {
...
/// 重置单个发射器为初始备份值。
pub(super) fn reset_single_emitter(cx: &mut Context<AppSidebar>, idx: usize) {
...
/// 重置整个系统为初始备份定义并重播。
pub(super) fn reset_system(cx: &mut Context<AppSidebar>) {
    update_state(|s| {
