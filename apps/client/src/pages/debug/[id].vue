...
import { createLogContext, LOG_CONTEXT_KEY } from "@/composables/useLogPoller";
...
import type { UnsubscribeFn } from "@/services/types";
import { StopCircleIcon, Loader2Icon } from "@lucide/vue";

const route = useRoute();
const router = useRouter();
const { t } = useLocale();

const gameId = computed(() => String((route.params as any).id));
const ws = useWsClient();
const log = createLogContext();
provide(LOG_CONTEXT_KEY, log);
...
function toggleGodMode() {
  ws.send("god_mode", { enabled: !gameState.value.godMode });
}

function toggleCooldown() {
  ws.send("toggle_cooldown", { enabled: !gameState.value.cooldownDisabled });
}

function togglePause() {
  ws.send("toggle_pause", {});
}

function resetPosition() {
  ws.send("reset_position", {});
}

function switchChampion() {
  ws.send("switch_champion", { name: switchTarget.value });
}
...
async function handleStopGame() {
  try {
    await services.local.stopGame(gameId.value);
  } catch (e) {
...
onMounted(async () => {
  try {
    const game = await services.local.getRunningGame(gameId.value);
    if (!game) {
      error.value = "对局未运行或不存在";
      loading.value = false;
      return;
    }
    await ws.connect(gameId.value);
    await log.start(gameId.value);
    loading.value = false;
  } catch (err: any) {
    error.value = err.message || (typeof err === "string" ? err : "连接 Bevy 仿真对局失败");
    loading.value = false;
  }

  unlistenHistory = await services.local.onAgentHistoryUpdated((payload) => {
...
<template>
  <div class="bg-background flex h-full w-full flex-col overflow-hidden">
    ...
    <div v-if="loading" class="flex flex-1 flex-col items-center justify-center gap-3">
      ...
      <p class="text-muted-foreground text-sm">正在连接 Bevy 仿真对局 (ID: {{ gameId.slice(0, 8) }})…</p>
    </div>
    <div v-else-if="error" class="flex flex-1 flex-col items-center justify-center gap-3">
      ...
      <Button variant="outline" size="sm" @click="router.push('/')">返回首页</Button>
    </div>

    <!-- 正常工作区 -->
    <div v-else class="flex h-full flex-col gap-3 p-4 overflow-hidden">
      <!-- Status Bar -->
