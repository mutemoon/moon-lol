...
import { BotIcon, PlusIcon, Trash2Icon, SaveIcon, ArrowLeftIcon, InfoIcon } from "@lucide/vue";
    ...
    <div class="mt-3 flex min-h-0 flex-1 gap-3">
      ...
      <section class="border-border bg-card min-w-0 flex-1 overflow-y-auto rounded-lg border p-5 shadow-sm">
        ...
        <div class="mx-auto flex max-w-xl flex-col gap-4">
          ...
          <!-- 说明：Agent 预设英雄无关 -->
          <div
            class="border-primary/20 bg-primary/5 text-foreground/70 flex items-start gap-1.5 rounded-md border px-3 py-2 text-[11px] leading-relaxed"
          >
            <InfoIcon class="text-primary mt-0.5 size-3.5 shrink-0" />
            <span v-html="t('agents.info')"></span>
          </div>

          <!-- 类型说明 -->
          <div
            v-if="currentTypeMeta?.soon"
            class="flex items-start gap-1.5 rounded-md border border-amber-500/30 bg-amber-500/5 px-3 py-2 text-[11px] leading-relaxed text-amber-500"
          >
      ...
      <DialogContent class="border-border bg-card text-foreground max-w-sm p-6">
        <DialogHeader>
          <DialogTitle class="text-foreground text-sm">
            {{ t("agents.deleteConfirmTitle", { name: selectedName }) }}
          </DialogTitle>
