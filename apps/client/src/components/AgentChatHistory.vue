...
<template>
  <div class="flex h-full w-full flex-col overflow-hidden">
    <!-- ==================== 顶部悬浮筛选过滤控制面板 ==================== -->
    <div
      class="border-border bg-card/90 sticky top-0 z-30 flex shrink-0 flex-col gap-3 border-b px-1 pt-1 pb-4 backdrop-blur-md"
    >
      <!-- 第一排：搜索、轮次筛选与全局折叠 -->
      <div class="flex flex-wrap items-center justify-between gap-3">
        <!-- 搜索输入框 -->
        <div class="relative min-w-[200px] flex-1">
          <SearchIcon class="text-muted-foreground/50 absolute top-2.5 left-3 h-4 w-4" />
          <Input
            v-model="searchQuery"
            placeholder="搜寻思维、工具指令、返回数据..."
            class="bg-muted/40 border-border/40 focus-visible:border-primary/50 focus-visible:ring-primary/10 placeholder:text-muted-foreground/40 text-foreground h-9 w-full pr-8 pl-9 text-xs"
          />
          <button
            v-if="searchQuery"
            class="text-muted-foreground hover:text-foreground absolute top-2.5 right-3 cursor-pointer text-xs"
            @click="searchQuery = ''"
          >
            ✕
          </button>
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <div class="flex items-center gap-1.5">
            ...
            <Select v-model="roundFilter">
              <SelectTrigger
                class="bg-muted/40 border-border/40 text-foreground h-8 w-[100px] text-[11px] font-semibold"
              >
                <SelectValue placeholder="所有轮次" />
              </SelectTrigger>
              <SelectContent class="border-border bg-popover text-popover-foreground text-xs">
          ...
          <div class="flex items-center gap-1.5">
            <Button
              variant="outline"
              size="xs"
              class="border-primary/20 hover:bg-primary/10 hover:text-primary text-muted-foreground h-8 cursor-pointer gap-1 rounded px-2.5 text-[11px] transition-all"
              @click="defaultCollapseThoughts = !defaultCollapseThoughts"
            >
              <HelpCircleIcon class="text-primary h-3 w-3" />
            ...
            <Button
              variant="outline"
              size="xs"
              class="text-muted-foreground h-8 cursor-pointer gap-1 rounded border-blue-500/20 px-2.5 text-[11px] transition-all hover:bg-blue-500/10 hover:text-blue-400"
      ...
      <div class="flex flex-wrap items-center gap-2 text-xs">
        <span class="text-muted-foreground mr-1 text-[10px] font-medium select-none">过滤内容:</span>

        <!-- 思维过程 Toggle Button -->
        <button
          @click="showThoughts = !showThoughts"
          :class="[
            'flex cursor-pointer items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-semibold shadow-xs transition-all duration-200 outline-none select-none',
            showThoughts
        ...
        >
          <span
            ...
            :class="showThoughts ? 'bg-primary animate-pulse' : 'bg-muted-foreground/60'"
          ></span>
          思维链 (Thoughts)
        </button>
        ...
        <button
          ...
          :class="[
            'flex cursor-pointer items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-semibold shadow-xs transition-all duration-200 outline-none select-none',
            showTools
              ? 'border-blue-500/40 bg-blue-500/10 text-blue-400 shadow-inner'
              : 'bg-muted/20 border-border text-muted-foreground hover:text-foreground',
          ]"
        >
          <span
            class="h-1.5 w-1.5 rounded-full"
            :class="showTools ? 'animate-pulse bg-blue-400' : 'bg-muted-foreground/60'"
          ></span>
          工具与输出 (Tools)
        </button>
        ...
        <button
          ...
          :class="[
            'flex cursor-pointer items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-semibold shadow-xs transition-all duration-200 outline-none select-none',
            showDecisions
              ? 'border-purple-500/40 bg-purple-500/10 text-purple-400 shadow-inner'
              : 'bg-muted/20 border-border text-muted-foreground hover:text-foreground',
          ]"
        >
          <span
            class="h-1.5 w-1.5 rounded-full"
            :class="showDecisions ? 'animate-pulse bg-purple-400' : 'bg-muted-foreground/60'"
          ></span>
          公开决策 (Replies)
        </button>
        ...
        <button
          ...
          :class="[
            'flex cursor-pointer items-center gap-1.5 rounded-full border px-3 py-1 text-[11px] font-semibold shadow-xs transition-all duration-200 outline-none select-none',
            showObservations
              ? 'border-zinc-500/40 bg-zinc-500/10 text-zinc-300 shadow-inner'
              : 'bg-muted/20 border-border text-muted-foreground hover:text-foreground',
          ]"
        >
          <span
            class="h-1.5 w-1.5 rounded-full"
            :class="showObservations ? 'animate-pulse bg-zinc-400' : 'bg-muted-foreground/60'"
          ></span>
          环境观测 (Observ.)
        </button>

        <!-- 数量指示器 -->
        <span class="text-muted-foreground ml-auto font-mono text-[10px] select-none">
    ...
    <!-- ==================== 历史流滚动区域 ==================== -->
    <ScrollArea class="min-h-0 w-full flex-1 pr-2">
      <!-- 空数据或 Loading 状态 -->
      <div
      ...
      >
        <svg
          class="h-8 w-8 animate-pulse text-zinc-600 opacity-60"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
        >
      ...
      <!-- 消息历史流 -->
      <div v-else class="flex flex-col gap-5 pt-2 pb-6">
        <div v-for="msg in filteredMessages" :key="msg.originalIndex" class="flex flex-col gap-2">
          <!-- ==================== USER ROLE ==================== -->
          <template v-if="msg.role === 'user'">
            <div class="mt-4 flex items-center gap-3">
              <div class="bg-border h-px flex-1"></div>
              <div
                class="border-primary/25 text-primary bg-muted flex items-center gap-1.5 rounded-full border px-4 py-0.5 font-mono text-[9px] tracking-wider uppercase shadow-sm select-none"
              >
            ...
            <div class="flex flex-col gap-2.5 px-1">
              <template v-for="(item, itemIdx) in msg.items" :key="itemIdx">
                ...
                <div
                  v-if="item.type === 'text'"
                  class="text-foreground border-border/30 bg-card max-w-4xl rounded-r-lg rounded-bl-lg border p-3 text-xs leading-relaxed shadow-sm"
                >
                ...
                >
                  <!-- 终端标题栏 -->
                  <div
                    class="border-border/30 bg-muted flex items-center justify-between border-b px-3 py-1.5 text-[9px]"
                  >
                    <div class="text-muted-foreground flex items-center gap-1.5 select-none">
                      <span
                        class="h-2 w-2 rounded-full"
                        :class="item.isError ? 'bg-destructive/60' : 'bg-green-500/60'"
                      ></span>
                    ...
                    <div class="flex items-center gap-2 select-none">
                      ...
                      <button
                        ...
                        class="text-muted-foreground hover:text-foreground bg-muted/80 flex cursor-pointer items-center gap-0.5 rounded px-1.5 py-0.5 transition-colors"
                      >
                        <span class="text-[8px] font-bold">
                          {{ isItemCollapsed(msg.originalIndex, itemIdx, "toolresult") ? "展开" : "折叠" }}
                        </span>
                        <ChevronDownIcon
                  ...
                  <div
                    ...
                    class="p-3 text-[11px] leading-relaxed whitespace-pre-wrap select-text"
                  ...
                  <div
                    v-show="isItemCollapsed(msg.originalIndex, itemIdx, 'toolresult')"
                    @click="toggleItemCollapse(msg.originalIndex, itemIdx, 'toolresult')"
                    class="text-muted-foreground bg-muted/20 flex cursor-pointer items-center justify-between p-2.5 text-[10px] italic transition-colors select-none"
                  >
                    <span>终端输出已折叠 ({{ item.text ? item.text.split("\n").length : 0 }} 行内容 · 点击展开)</span>
                    <span class="text-primary/80 font-semibold group-hover:underline">展开终端结果</span>
                  </div>
                </div>
              </template>
            </div>
            ...
            <div class="flex flex-col gap-3">
              <template v-for="(item, itemIdx) in msg.items" :key="itemIdx">
                ...
                >
                  ...
                  <div class="mb-2 flex items-center justify-between select-none">
                    <div class="text-primary flex items-center gap-1.5 text-[11px] font-semibold tracking-wide">
                      <BrainIcon class="text-primary h-3.5 w-3.5 animate-pulse" />
                      <span>{{ champion }} 的内部思维链 (Thought reasoning)</span>
                      <span
                        v-if="isItemCollapsed(msg.originalIndex, itemIdx, 'thought')"
                        class="text-muted-foreground text-[10px] font-normal italic"
                      >
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      <button
                        ...
                        class="text-primary hover:text-primary/80 bg-primary/10 hover:bg-primary/20 flex cursor-pointer items-center gap-0.5 rounded px-1.5 py-0.5 transition-colors"
                      >
                        <span class="text-[9px] font-bold">
                          {{ isItemCollapsed(msg.originalIndex, itemIdx, "thought") ? "展开" : "折叠" }}
                        </span>
                        <ChevronDownIcon
                  ...
                  <div
                    ...
                    class="text-muted-foreground font-sans text-xs leading-relaxed select-text"
                  >
                    <MarkdownRender :content="item.text || ''" :smooth-streaming="false" :fade="true" />
                  </div>
                  <!-- 折叠简略预览提示 -->
                  <div
                    v-show="isItemCollapsed(msg.originalIndex, itemIdx, 'thought')"
                    @click="toggleItemCollapse(msg.originalIndex, itemIdx, 'thought')"
                    class="text-primary/80 hover:text-primary bg-primary/5 border-primary/10 flex cursor-pointer items-center justify-between rounded border px-2.5 py-2 text-[11px] italic transition-colors"
                  >
                    <span class="text-muted-foreground truncate pr-4 select-none">
                      {{ item.text ? item.text.replace(/[\n\s#*`>]/g, " ").slice(0, 75) : "" }}...
                    </span>
                    <span class="text-primary flex shrink-0 items-center gap-1 font-semibold select-none">
                      展开思维链
                    </span>
                  </div>
                </div>
                ...
                <div
                  ...
                  class="flex flex-col overflow-hidden rounded border border-blue-500/20 bg-blue-500/5 font-mono transition-all duration-300"
                >
                  <!-- 工具调用标题 -->
                  <div
                    class="flex items-center justify-between border-b border-blue-500/10 bg-blue-500/10 px-3.5 py-1.5 text-[10px] font-semibold text-blue-500 select-none"
                  >
                    <div class="flex items-center gap-1.5">
                    ...
                    <Badge
                      variant="outline"
                      class="border-blue-400/30 bg-blue-500/10 px-1 py-0 text-[8px] text-blue-500"
                    >
                      Active Call
                    </Badge>
                  </div>
                  <!-- 调用的命令行内容 -->
                  <div class="bg-muted text-foreground flex items-center gap-2 p-3 text-xs leading-relaxed select-text">
                    <span class="font-bold text-blue-500 select-none">$</span>
                    <span class="flex-1 font-mono">{{ item.command }}</span>
                  </div>
                </div>
                ...
                <div
                  ...
                  class="border-border/40 bg-card relative overflow-hidden rounded border p-4 font-sans text-xs leading-relaxed shadow-sm transition-all duration-300"
                >
                  <div
                    class="from-primary/5 pointer-events-none absolute top-0 right-0 h-24 w-24 rounded-full bg-gradient-to-br to-transparent blur-xl"
                  ></div>

                  <div class="border-border/10 mb-2.5 flex items-center justify-between border-b pb-1.5 select-none">
                    <span class="text-primary text-[11px] font-bold tracking-wide">{{ champion }} 的公开对线决策</span>
                    <span class="text-muted-foreground font-mono text-[9px] opacity-75">Assistant Reply</span>
                  </div>

                  <div class="text-foreground pr-1.5 leading-relaxed font-medium select-text">
                    <MarkdownRender :content="item.text || ''" :smooth-streaming="false" :fade="true" />
                  </div>
                </div>
              </template>
