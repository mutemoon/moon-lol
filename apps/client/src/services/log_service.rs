//! 日志查询服务。
//!
//! 仿 `apps/client/src-tauri/src/log.rs` 的 Tauri 命令实现，
//! 对每局独立的 SQLite 日志文件执行只读查询（query_logs / query_log_entities /
//! query_log_categories）与清理（clear_logs）。
...
use super::types::{LogCategory, LogEntity, LogQueryParams, LogRow, QueryLogsResult};
...
/// 查询日志列表（分页、按级别/实体/类别/关键字筛选）。
///
/// 对齐 Tauri 端 `query_logs` 命令的 SQL 与绑定参数，一模一样。
pub async fn query_logs(game_id: &str, params: &LogQueryParams) -> Result<QueryLogsResult, String> {
    let game_id = game_id.to_string();
    let params = params.clone();
    super::runtime::run_on_tokio(move || async move {
        let db_path = match log_db_path(&game_id) {
            Some(p) => p,
            None => {
...
/// 查询对局中所有出现过的实体（entity_id + entity_name）。
pub async fn query_log_entities(game_id: &str) -> Result<Vec<LogEntity>, String> {
    let game_id = game_id.to_string();
    super::runtime::run_on_tokio(move || async move {
        let db_path = match log_db_path(&game_id) {
            Some(p) => p,
...
/// 查询对局中所有出现过的日志类别。
pub async fn query_log_categories(game_id: &str) -> Result<Vec<LogCategory>, String> {
    let game_id = game_id.to_string();
    super::runtime::run_on_tokio(move || async move {
        let db_path = match log_db_path(&game_id) {
            Some(p) => p,
...
/// 清空指定对局的日志表。
pub async fn clear_logs(game_id: &str) -> Result<(), String> {
    let game_id = game_id.to_string();
    super::runtime::run_on_tokio(move || async move {
        let db_path = match log_db_path(&game_id) {
            Some(p) => p,
