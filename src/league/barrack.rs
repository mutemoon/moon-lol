...
impl LeagueLoader {
    ...
    pub async fn save_barrack_async(
        &self,
        value: &BinStruct,
    ) -> Result<(Transform, Team, Lane, Barrack), LeagueLoaderError> {
        let transform = value
        ...
        let units_list = value
            .getv::<BinList>(LeagueLoader::hash_bin("units").into())
            .unwrap();

        // 并行处理 minion 配置
        let minion_tasks: Vec<_> = units_list
            .downcast::<BinEmbed>()
            .unwrap()
            .iter()
            .map(|embed| self.load_barrack_minion_config_async(embed))
            .collect();

        let units = join_all(minion_tasks)
            .await
            .into_iter()
    ...
    async fn load_barrack_minion_config_async(
        &self,
        value: &BinEmbed,
    ) -> Result<BarracksMinionConfig, LeagueLoaderError> {
        let minion_type = value
