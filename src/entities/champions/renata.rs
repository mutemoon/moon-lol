...
use crate::{BuffRenataQ, BuffRenataR, BuffRenataW, DamageType, PassiveSkillOf};
...
fn cast_renata_w(commands: &mut Commands, entity: Entity) {
    ...
    commands.entity(entity).with_related::<BuffOf>(BuffRenataW::new(0.5, 5.0));
}
...
fn cast_renata_e(commands: &mut Commands, entity: Entity) {
    ...
    skill_damage(
        ...
        entity,
        RENATA_E_KEY,
        DamageShape::Sector { radius: 800.0, angle: 20.0 },
        vec![TargetDamage {
            filter: TargetFilter::All,
            amount: hash_bin("TotalDamage"),
            damage_type: DamageType::Magic,
...
fn cast_renata_r(commands: &mut Commands, entity: Entity) {
    ...
    skill_damage(
        ...
        entity,
        RENATA_R_KEY,
        DamageShape::Sector { radius: 1500.0, angle: 60.0 },
        vec![TargetDamage {
            filter: TargetFilter::All,
            amount: hash_bin("TotalDamage"),
            damage_type: DamageType::Magic,
...
fn on_renata_damage_hit(
    ...
    // Q slows
    commands.entity(target).with_related::<BuffOf>(BuffRenataQ::new(0.5, 1.5));
    // R stuns
    commands.entity(target).with_related::<BuffOf>(BuffRenataR::new(0.75, 1.0));
}
