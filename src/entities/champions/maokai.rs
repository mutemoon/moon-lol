...
use crate::{BuffMaokaiW, DebuffSlow, DamageType, PassiveSkillOf};
...
fn cast_maokai_r(commands: &mut Commands, entity: Entity) {
    ...
    skill_damage(
        ...
        entity,
        MAOKAI_R_KEY,
        DamageShape::Sector { radius: 3000.0, angle: 45.0 },
        vec![TargetDamage {
            filter: TargetFilter::All,
            amount: hash_bin("TotalDamage"),
            damage_type: DamageType::Magic,
...
fn on_maokai_damage_hit(
    ...
    // W roots
    commands.entity(target).with_related::<BuffOf>(BuffMaokaiW::new(2.0, 2.0));

    // E slows
    commands.entity(target).with_related::<BuffOf>(DebuffSlow::new(0.35, 3.0));
}
