...
use crate::{BuffSionE, BuffSionQ, DamageType, PassiveSkillOf};
...
fn cast_sion_e(commands: &mut Commands, entity: Entity) {
    ...
    skill_damage(
        ...
        entity,
        SION_E_KEY,
        DamageShape::Sector { radius: 750.0, angle: 15.0 },
        vec![TargetDamage {
            filter: TargetFilter::All,
            amount: hash_bin("TotalDamage"),
            damage_type: DamageType::Physical,
        }],
        Some(hash_bin("Sion_E_Hit")),
    );
}
...
fn on_sion_damage_hit(
    ...
    // Q knocks up
    commands.entity(target).with_related::<BuffOf>(BuffSionQ::new(0.5, 1.0));
    // E slows
    commands.entity(target).with_related::<BuffOf>(BuffSionE::new(0.4, 2.0));
}
