...
use crate::{BuffBriarPassive, BuffBriarQ, BuffBriarW, DamageType, PassiveSkillOf};
...
fn cast_briar_r(commands: &mut Commands, entity: Entity) {
    ...
    skill_damage(
        ...
        entity,
        BRIAR_R_KEY,
        DamageShape::Nearest { max_distance: 12000.0 },
        vec![TargetDamage {
            filter: TargetFilter::All,
            amount: hash_bin("TotalDamage"),
            damage_type: DamageType::Magic,
...
fn on_briar_damage_hit(
    ...
    commands.entity(target).with_related::<BuffOf>(BuffBriarPassive::new(1, 10.0, 6.0));
    commands.entity(target).with_related::<BuffOf>(BuffBriarQ::new(0.85, 15.0, 2.0));
    commands.entity(target).with_related::<BuffOf>(BuffBriarW::new(0.75, 0.4, 4.0));
}
